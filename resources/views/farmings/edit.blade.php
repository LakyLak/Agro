...
@section('content')
    <div class="container-fluid">
        <div class="row">
            <h4 class="page-title">Update Farming</h4>
            <div class="col-md-12">
                <form class="form-horizontal" method="post" action="{{ url('/farmings/edit/'. $farming->id) }}" 
                    name="edit_farming" id="edit_farming" novalidate="novalidate">{{ csrf_field() }}
                    <div class="card">
                        <div class="card-body">
                                ...
                                <label for="tractor" class="col-sm-3 control-label col-form-label">Tractor Name</label>
                                    ...
                                    <select class="select2 form-control custom-select {{ $errors->has('tractor') ? 'is-invalid' : '' }}" 
                                        ...
                                        @foreach ($tractors as $tractor)
                                            <option value="{{ $tractor->id }}" {{ $tractor->id == $farming->tractor_id ? ' selected' : '' }}>
                                                {{ $tractor->name }} 
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <label for="field" class="col-sm-3 control-label col-form-label">Field Name</label>
                                    ...
                                    <select class="select2 form-control custom-select {{ $errors->has('field') ? 'is-invalid' : '' }}" 
                                        ...
                                        @foreach ($fields as $field)
                                            <option value="{{ $field->id }}" {{ $field->id == $farming->field_id ? ' selected' : '' }}>
                                                {{ $field->name }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-sm-9 input-group">
                                    <input type="text" class="form-control mydatepicker {{ $errors->has('date') ? 'is-invalid' : '' }}" 
                                        id="date" name="date" placeholder="yyyy-mmm-dd" value="{{ $farming->date ?? '' }}">
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-sm-9">
                                    <input type="text" class="form-control {{ $errors->has('farming_area') ? 'is-invalid' : '' }}" 
                                        id="farming_area" name="farming_area" placeholder="Farming Area Here" value="{{ $farming->farming_area ?? '' }}">
                        ...
                        <div class="border-top">
                            <div class="card-body">
                                <button type="submit" class="btn btn-primary">Update Farming</button>
                            </div>
                        </div>
                    </div>
                </form>
